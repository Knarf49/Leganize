      ...
      {threads.map((t) => {
        ...
        return (
          <div key={t.thread_id} className="w-full px-1">
            <Button
              variant="ghost"
              className="text-left items-start justify-start font-normal w-70"
              onClick={(e) => {
                e.preventDefault();
  ...
  return (
    <div className="h-full flex flex-col w-full gap-2 items-start justify-start overflow-y-scroll [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-300 [&::-webkit-scrollbar-track]:bg-transparent">
      {Array.from({ length: 30 }).map((_, i) => (
        <Skeleton key={`skeleton-${i}`} className="w-70 h-10" />
      ))}
    </div>
  );
}
...
export default function ThreadHistory() {
  ...
  return (
    <>
      <div className="hidden lg:flex flex-col border-r border-slate-300 items-start justify-start gap-6 h-screen w-75 shrink-0 shadow-inner-right">
